...
function BandsTable() {
  ...
  const handleDetailFilter = React.useCallback(
    (val) => {
      if (val) {
  ...
  const playRecommendedTrackOrOpenLink = (row) => {
    ...
    if (row.links) {
      setTimeout(() => window.open(row.links, "_blank"), 10);
    }
  };
    ...
    if (column.deezerId || column.deezerRecommendationId) {
      return (
        <span className="flex justify-center">
          <PlayCircle />
          <VisuallyHidden.Root>
            This band has a preview that can be played clicking on the row.
          </VisuallyHidden.Root>
        </span>
      );
    }
    if (column.links && !column.deezerId && !column.deezerRecommendationId) {
      return (
        ...
        >
          <ExternalLink />
          <VisuallyHidden.Root>
            Link for an official page or a track from the band.
          </VisuallyHidden.Root>
        </a>
      );
    }
  ...
  const columns = React.useMemo(() => {
    const cols: TableColumn[] = [
      {
        visible: true,
        formatElement: formatPlayOrLink,
        headerLabel: "Play/Link"
      },
      {
        filter: true,
        visible: true,
      ...
      },
      {
        ...
        visible: true,
        field: "growling",
        headerLabel: "Growling",
        sortable: true,
        tagList: growTagList,
        tag: true,
        sort: "desc",
        sortWithRawValue: true
      },
      {
        filter: false,
        visible: true,
      ...
      },
      {
        ...
        visible: false,
        field: "blackWomen",
        headerLabel: "Black Women",
        sortable: true,
        tagList: booleanTagList,
        tag: true
      },
      {
        filter: false,
      ...
      },
      {
        filter: true,
        visible: true,
        field: "currentVocalists",
        headerLabel: "Vocalists"
      },
      {
        filter: true,
        visible: true,
  ...
  return (
    <section>
      <div className="gap-4 flex-col flex md:gap-8 items-center">
        <h2 className="title1">The List</h2>

        <div className="flex flex-col gap-2 text-center">
          <p className="title2">{total} bands</p>
          {total !== totalFiltered && (
            <small className="title2">(filtered: {totalFiltered} bands)</small>
          )}
        </div>
        ...
        {!isLoading && (
          <div className="flex flex-col lg:flex-row text-center md:text-left md:justify-between items-center mb-6 gap-3">
            <p className="label m-0">
              Click on a row to play a preview or to open the band's website.
            </p>
          ...
          <div className="flex flex-col lg:flex-row lg:items-center gap-3">
            <p className="label md:mb-0">
              ...
              <span className="font-black">
                {currentPage * size} - {(currentPage + 1) * size}
              </span>{" "}
              of <span className="font-black">{totalFiltered}</span> items
            </p>
            <Pagination
              currentPage={currentPage}
              lastPage={lastPage}
